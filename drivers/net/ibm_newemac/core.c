...
#if defined(CONFIG_BGP_SUPPRESS_CHECKSUMS) || \
	defined(CONFIG_EMAC_MULTICAST_TAGGING)
// Needed for clearing checksum fields in outgoing packets.  Yuck.
#include <linux/in.h>
#include <linux/ip.h>
#include <linux/udp.h>
#include <linux/tcp.h>
#endif /* CONFIG_BGP_SUPPRESS_CHECKSUMS || CONFIG_EMAC_MULTICAST_TAGGING */
...
static inline int emac_phy_supports_10gige(int phy_mode)
{
	return phy_mode == PHY_MODE_XGMII;
}
...
static inline void emac_tx_enable(struct emac_instance *dev)
{
	...
	if (emac_has_feature(dev, EMAC_FTR_XEMAC))
		out_be32(&p->tmr0, XEMAC_TMR0_XMIT);
}
...
static inline u32 emac_iff2rmr(struct net_device *ndev)
{
	...
	if (emac_has_feature(dev, EMAC_FTR_EMAC4))
	    r |= EMAC4_RMR_BASE;
	else if (emac_has_feature(dev, EMAC_FTR_XEMAC))
	    r |= XEMAC_RMR_BASE;
	else
	    r |= EMAC_RMR_BASE;
...
static u32 __xemac_calc_base_mr1(struct emac_instance *dev, int tx_size, int rx_size)
{
	u32 ret = XEMAC_MR1_SDR | XEMAC_MR1_TRQ | XEMAC_MR1_MWSW_001 |
		XEMAC_MR1_OBCI(dev->opb_bus_freq);

	DBG2(dev, "__xemac_calc_base_mr1" NL);

	switch(tx_size) {
	case (64 * 1024): ret |= XEMAC_MR1_TFS_64K; break;
	case (32 * 1024): ret |= XEMAC_MR1_TFS_32K; break;
	case (16 * 1024): ret |= XEMAC_MR1_TFS_16K; break;
	case ( 8 * 1024): ret |= XEMAC_MR1_TFS_8K; break;
	case ( 4 * 1024): ret |= XEMAC_MR1_TFS_4K; break;
	case ( 2 * 1024): ret |= XEMAC_MR1_TFS_2K; break;
	default:
		printk(KERN_WARNING "%s: Unknown Rx FIFO size %d\n",
		       dev->ndev->name, tx_size);
	}
	...
	switch(rx_size) {
	case (64 * 1024): ret |= XEMAC_MR1_RFS_64K; break;
	case (32 * 1024): ret |= XEMAC_MR1_RFS_32K; break;
	case (16 * 1024): ret |= XEMAC_MR1_RFS_16K; break;
	case ( 8 * 1024): ret |= XEMAC_MR1_RFS_8K; break;
	case ( 4 * 1024): ret |= XEMAC_MR1_RFS_4K; break;
	case ( 2 * 1024): ret |= XEMAC_MR1_RFS_2K; break;
	default:
		printk(KERN_WARNING "%s: Unknown Rx FIFO size %d\n",
		       dev->ndev->name, rx_size);
	}
...
{
	if (emac_has_feature(dev, EMAC_FTR_EMAC4))
		return __emac4_calc_base_mr1(dev, tx_size, rx_size);
	else if (emac_has_feature(dev, EMAC_FTR_XEMAC))
		return __xemac_calc_base_mr1(dev, tx_size, rx_size);
	else
		return __emac_calc_base_mr1(dev, tx_size, rx_size);
}
...
static int emac_configure(struct emac_instance *dev)
{
	...
	if (dev->phy.duplex == DUPLEX_FULL) {
		mr1 |= EMAC_MR1_MWSW_001;
		if (!emac_has_feature(dev, EMAC_FTR_XEMAC))
			mr1 |= EMAC_MR1_FDE;
	}

	/* XEmac only supports 10GBit */
	if (emac_has_feature(dev, EMAC_FTR_XEMAC))
		dev->phy.speed = SPEED_10000;

	/* Adjust fifo sizes, mr1 and timeouts based on link speed */
	dev->stop_timeout = STOP_TIMEOUT_10;
	switch (dev->phy.speed) {
	case SPEED_10000:
		DBG(dev, "speed 10000, mtu=%d\n", dev->ndev->mtu);
	...
	{
		r = XEMAC_TMR1((dev->mal_burst_size / dev->fifo_entry_size) + 1,
			       tx_size / 2 / dev->fifo_entry_size);
		r = 0x02200240;
	}
	else
	...
	r = EMAC_ISR_OVR | EMAC_ISR_BP | EMAC_ISR_SE |
		EMAC_ISR_BFCS | EMAC_ISR_PTLE | EMAC_ISR_ORE |
	...
	if (emac_has_feature(dev, EMAC_FTR_EMAC4) ||
	    emac_has_feature(dev, EMAC_FTR_XEMAC))
...
static void emac_full_tx_reset_with_mal_reinit(struct emac_instance *dev)
{
	DBG(dev, "full_tx_reset_with_mal_reinit" NL);
	...
	/*
	 * THIS WILL BREAK OTHER CHANNELS.
	 * It also resets the rx engine, so the calling code has to have
	 * disabled rx, and has to reset rx_slot, etc.
	 */
	mal_reinit(dev->mal);
	emac_configure(dev);
...
static int __emac_mdio_read(struct emac_instance *dev, u16 id, u16 reg)
{
	struct emac_regs __iomem *p = dev->emacp;
	u32 r = 0;
	int n, err = -ETIMEDOUT;
	...
	DBG2(dev, "mdio_read(%02x,%04x), stacr=%p" NL, id, reg, &p->stacr);
	...
	if (emac_has_feature(dev, EMAC_FTR_HAS_XGMII))
		xgmii_get_mdio(dev->xgmii_dev, dev->xgmii_port);
	...
	/* indirect mode */
	if (emac_has_feature(dev, EMAC_FTR_INDIRECT_STACR)) {
		// issue address command
		r = EMACX_STACR_STAC_IND_ADDR | (reg << 16 | id);
		if (emac_has_feature(dev, EMAC_FTR_STACR_OC_INVERT))
			r |= EMAC_STACR_OC;
		out_be32(&p->stacr, r);

		/* Wait for address to complete */
		n = 100;
		...
		if (unlikely(r & EMAC_STACR_PHYE)) {
			DBG(dev, "mdio_address(%02x, %04x) (%04x) failed" NL, id, reg, r);
			err = -EREMOTEIO;
			goto bail;
		}
	}
	...
	if (emac_has_feature(dev, EMAC_FTR_STACR_OC_INVERT))
		r |= EMAC_STACR_OC;
	if (emac_has_feature(dev, EMAC_FTR_INDIRECT_STACR)) {
		r |= EMACX_STACR_STAC_IND_READ | id;
	} else {
	...
	if (unlikely(r & EMAC_STACR_PHYE)) {
		DBG(dev, "mdio_read(%02x, %04x) (%04x) failed" NL, id, reg, r);
		err = -EREMOTEIO;
		goto bail;
	}
 ...
 bail:
	if (emac_has_feature(dev, EMAC_FTR_HAS_XGMII))
		xgmii_put_mdio(dev->xgmii_dev, dev->xgmii_port);
	if (emac_has_feature(dev, EMAC_FTR_HAS_RGMII))
...
static void __emac_mdio_write(struct emac_instance *dev, u8 id, u16 reg,
			      u16 val)
{
	struct emac_regs __iomem *p = dev->emacp;
	u32 r = 0;
	...
	DBG2(dev, "mdio_write(%02x,%04x,%04x)" NL, id, reg, val);
	...
	if (emac_has_feature(dev, EMAC_FTR_INDIRECT_STACR))
	{
		r |= EMACX_STACR_STAC_IND_WRITE | (val << 16 | id);
	} else {
		if (emac_has_feature(dev, EMAC_FTR_HAS_AXON_STACR))
...
static int emac_resize_rx_ring(struct emac_instance *dev, int new_mtu)
{
	...
	int rx_size = emac_rx_size(new_mtu);
	int i, ret = 0;
...
// VU: how do we handle this in a generic fashion???
		if (dev->rx_desc[i].ctrl & MAL_RX_CTRL_FIRST)
			++dev->estats.rx_dropped_resize;
#endif
		mal_set_rx_desc(&dev->rx_desc[i], dev->rx_desc[i].data_ptr,
				dev->rx_size, i == (NUM_RX_BUFF - 1));
	}
	...
	for (i = 0; i < NUM_RX_BUFF; ++i) {
		...
		data_ptr = dma_map_single(&dev->ofdev->dev, skb->data - 2, rx_sync_size,
					  DMA_FROM_DEVICE) + 2;

		mal_set_rx_desc(&dev->rx_desc[i], data_ptr, rx_size,
				i == (NUM_RX_BUFF - 1));
 ...
 skip:
	...
	/*
	 * We need to reset the channel rx engine, whether or not we're
	 * changing the "Jumbo" bit. The only way to do it is to reset the
	 * whole thing (rx and tx, all channels).
	 * THIS WILL BREAK OTHER CHANNELS.
	 */
	...
	emac_full_tx_reset_with_mal_reinit(dev);
	...
	mal_set_rcbs(dev->mal, dev->mal_rx_chan, rx_size);
	mal_announce_rxdesc(dev->mal, dev->mal_rx_chan, NUM_RX_BUFF, rx_size);
...
static int emac_change_mtu(struct net_device *ndev, int new_mtu)
{
	...
	if (!ret) {
		ndev->mtu = new_mtu;
		dev->rx_size = emac_rx_size(new_mtu);
...
static void emac_clean_tx_ring(struct emac_instance *dev)
{
	...
	for (i = 0; i < NUM_TX_BUFF; ++i) {
		if (dev->tx_skb[i]) {
			...
			dev->tx_skb[i] = NULL;
			if (!MAL_IS_TX_COMPLETED(&dev->tx_desc[i]))
				++dev->estats.tx_dropped;
		}
		mal_clear_desc(&dev->tx_desc[i]);
	}
}
...
static void emac_clean_rx_ring(struct emac_instance *dev)
{
	...
	printk("clean rx ring\n");

	for (i = 0; i < NUM_RX_BUFF; ++i)
		if (dev->rx_skb[i]) {
			dev_kfree_skb(dev->rx_skb[i]);
			dev->rx_skb[i] = NULL;
			mal_clear_desc(&dev->rx_desc[i]);
		}
				    ...
				    gfp_t flags)
{
	...
	skb_reserve(skb, EMAC_RX_SKB_HEADROOM + 2);
	addr = dma_map_single(&dev->ofdev->dev, skb->data - 2, dev->rx_sync_size,
			      DMA_FROM_DEVICE) + 2;

	mal_set_rx_desc(&dev->rx_desc[slot], addr, dev->rx_size,
			slot == (NUM_RX_BUFF - 1));

	mal_announce_rxdesc(dev->mal, dev->mal_rx_chan, 1, dev->rx_size);
...
static int emac_open(struct net_device *ndev)
{
	...
	mal_enable_tx_channel(dev->mal, dev->mal_tx_chan);
	mal_set_rcbs(dev->mal, dev->mal_rx_chan, dev->rx_size);
	mal_enable_rx_channel(dev->mal, dev->mal_rx_chan);
	emac_tx_enable(dev);
	emac_rx_enable(dev);
	emac_netif_start(dev);
			       ...
			       struct sk_buff *skb)
{
	...
	if (emac_has_feature(dev, EMAC_FTR_XEMAC)) {
		...
		/*
		 * We ask the device to generate checksums for ALL packets, not
		 * just those marked as CHECKSUM_PARTIAL.  We want the device
		 * to generate checksums for packets that arrived via bridging
		 * or ip routing from networks that didn't need checksums.
		 *
		 * The device will modify only those packets that are ipv4
		 * udp or tcp and whose udp/tcp checksum fields are clear.
		 * We have to parse the packets to clear the fields.  Yuck.
		 */
		if (skb->protocol == ETH_P_IP) {
			/*
			 * udp_hdr(skb) and tcp_hdr(skb) don't work here
			 * because skb->transport_header will not have been
			 * set properly for bridged packets.
			 */
			char *payload = ((char *) ip_hdr(skb)) +
					    (ip_hdr(skb)->ihl * sizeof(int));
			switch (ip_hdr(skb)->protocol) {
			case IPPROTO_UDP:
				ip_hdr(skb)->check = 0;
				//udp_hdr(skb)->check = 0;
				((struct udphdr *) payload)->check = 0;
				break;
			case IPPROTO_TCP:
				ip_hdr(skb)->check = 0;
				//tcp_hdr(skb)->check = 0;
				((struct tcphdr *) payload)->check = 0;
				break;
			default:
				break;
			}
		}
#else /* CONFIG_BGP_SUPPRESS_CHECKSUMS */
		if (skb->ip_summed == CHECKSUM_PARTIAL) {
			/*
			 * Linux won't ask us to checksum anything other than
			 * ipv4 udp and tcp packets, which is good because
			 * that's all TOMAL can handle.  The device requires
			 * us to clear the udp/tcp checksum field, whose
			 * location is indicated by csum_offset.
			 */
			*((__sum16 *) (skb->transport_header +
						skb->csum_offset)) = 0;
			++dev->stats.tx_packets_csum;
		}
#endif /* CONFIG_BGP_SUPPRESS_CHECKSUMS */
		++dev->stats.tx_packets_csum;
		return TOMAL_DESC_TX_CHECKSUM;
	}
#endif
	return 0;
}
...
static inline int emac_xmit_finish(struct emac_instance *dev, int len)
{
	...
	if (emac_has_feature(dev, EMAC_FTR_EMAC4))
		...
		mal_add_xmit(dev->mal, dev->mal_tx_chan, 1);
	else
...
static int emac_start_xmit(struct sk_buff *skb, struct net_device *ndev)
{
	...
	phys_addr_t data_ptr;
	int i;

	DBG2(dev, "xmit(%u) %d" NL, len, dev->tx_slot);

	dev->tx_skb[dev->tx_slot] = skb;

#if 0
	for (i = 0; i < len; i++)
		printk("%02x%c", ((unsigned char*)skb->data)[i], (i % 32) ? ' ' : '\n');
#endif

	data_ptr = dma_map_single(&dev->ofdev->dev, skb->data, len, DMA_TO_DEVICE);

	mal_set_tx_desc(&dev->tx_desc[dev->tx_slot], data_ptr, len, 1,
			(dev->tx_slot == NUM_TX_BUFF), emac_tx_csum(dev, skb));
...
static inline int emac_xmit_split(struct emac_instance *dev, int slot,
				  u32 pd, int len, int last, u16 flags)
{
	while (1) {
		...
		mal_set_tx_desc(&dev->tx_desc[slot], pd, chunk, (last && !len),
				(slot == NUM_TX_BUFF - 1), flags);
		++dev->tx_cnt;
...
static int emac_start_xmit_sg(struct sk_buff *skb, struct net_device *ndev)
{
	...
	len -= chunk;
	if (unlikely(len))
		slot = emac_xmit_split(dev, slot, pd + chunk, len, !nr_frags, 0);
	...
	for (i = 0; i < nr_frags; ++i) {
		...
		slot = emac_xmit_split(dev, slot, pd, len, i == nr_frags - 1, 0);
	}
	...
	/* Send the packet out */
	mal_set_tx_desc(&dev->tx_desc[dev->tx_slot], pd, (u16) chunk, 0,
			(dev->tx_slot == NUM_TX_BUFF - 1), emac_tx_csum(dev, skb));
 ...
 undo_frame:
	...
	while (slot != dev->tx_slot) {
		mal_clear_desc(&dev->tx_desc[slot]);
		--dev->tx_cnt;
		if (--slot < 0)
			slot = NUM_TX_BUFF - 1;
	}
...
static void emac_poll_tx(void *param)
{
	...
	again:
		if (MAL_IS_TX_COMPLETED(&dev->tx_desc[slot])) {
...
#ifndef CONFIG_IBM_NEW_EMAC_TOMAL
			if (unlikely(dev->tx_skb[slot].ctrl & bad_mask))
				mal_parse_tx_error(dev, &dev->tx_desc[slot]);
#endif
				       ...
				       int len)
{
	...
	mal_set_rx_desc(&dev->rx_desc[slot], dev->rx_desc[slot].data_ptr,
			dev->rx_size, slot == (NUM_RX_BUFF - 1));

	mal_announce_rxdesc(dev->mal, dev->mal_rx_chan, 1, dev->rx_size);
}


static inline int emac_rx_csum(struct emac_instance *dev,
				struct mal_descriptor *desc)
{
	int ret = CHECKSUM_NONE;
#ifdef CONFIG_IBM_NEW_EMAC_TAH
	if (!desc->ctrl && dev->tah_dev) {
		ret = CHECKSUM_UNNECESSARY;
		++dev->stats.rx_packets_csum;
	}
#elif defined(CONFIG_IBM_NEW_EMAC_TOMAL)
	if ((desc->status & TOMAL_DESC_RX_CHKSUM_MASK) ==
				TOMAL_DESC_RX_CHKSUM_PASSED) {
		ret = CHECKSUM_UNNECESSARY;
		++dev->stats.rx_packets_csum;
	}
#endif
...
#define EMAC_MCAST_TABLE_MAX 4

static struct {
    unsigned int group;
    atomic_t seqno;
} emac_mcast_table[EMAC_MCAST_TABLE_MAX];

static int emac_mcast_table_cur = 0;
			  ...
			  void __user *buffer, size_t *lenp, loff_t *ppos)
{
	...
	while (len > 0) {
	    ...
	    for (idx = 0; idx < emac_mcast_table_cur; idx++) {
		if (emac_mcast_table[idx].group == group) break;
	    }

	    if (idx == emac_mcast_table_cur) {
		/* adding entry */
		if (emac_mcast_table_cur >= EMAC_MCAST_TABLE_MAX) {
		    kfree(space);
		    return -ENOMEM;
		}
		emac_mcast_table[idx].group = group;
		emac_mcast_table_cur++;
	    }
	    atomic_set(&emac_mcast_table[idx].seqno, 0);
	}

	kfree(space);
    } else {
	space = kmalloc(1 + (emac_mcast_table_cur * 30), GFP_KERNEL);
	if (space == NULL) {
	    return -ENOMEM;
	}

	len = 0;
	for (i = 0; i < emac_mcast_table_cur; i++) {
	    snprintf(space + len, 30, "01:00:5E:%02X:%02X:%02X 0x%08x\n",
		     (emac_mcast_table[i].group >> 16) & 0x7f,
		     (emac_mcast_table[i].group >> 8) & 0xff,
		     (emac_mcast_table[i].group >> 0) & 0xff,
		     atomic_read(&emac_mcast_table[i].seqno));
	    len += 29;
	}
...
static int mcast_tag(struct emac_instance *dev, struct sk_buff *skb, int len)
{
    struct ethhdr *eth;
	...
	(eth->h_dest[1] == 0x00) &&
	(eth->h_dest[2] == 0x5E) &&
	((eth->h_dest[3] & 0x80) == 0x00) &&
	(eth->h_proto == ETH_P_IP))
    {
	unsigned int group;
	int i;

	group = (eth->h_dest[3] << 16) |
		(eth->h_dest[4] << 8) |
		(eth->h_dest[5] << 0);
	for (i = 0; i < emac_mcast_table_cur; i++) {
	    if (emac_mcast_table[i].group == group) {
		struct iphdr *ip;
		...
		if (ip->protocol == IPPROTO_UDP) {
		    ...
		    new_len = ((((unsigned char *) udp) + udp->len) -
							    skb->data) + 16;
		    if (new_len <= dev->ndev->mtu) {
			unsigned int tbu, tbl, tbc;
			unsigned int seqno;
			unsigned int *addendum;
			do {
			    asm("mftbu %0; mftbl %1; mftbu %2"
				: "=r" (tbu), "=r" (tbl), "=r" (tbc) :);
			} while (tbu != tbc);
			seqno = atomic_inc_return(&emac_mcast_table[i].seqno);
			addendum = (unsigned int *) (skb->data + new_len - 16);
			addendum[0] = htonl(0xadded001u); /* magic number */
			addendum[1] = htonl(seqno);
			addendum[2] = htonl(tbu);
			addendum[3] = htonl(tbl);
			udp->len += 16;
			ip->tot_len += 16;
			ip->check -= 16;
			/* one's complement fix-up on underflow */
			if (ip->check >= 0xfff0) ip->check--;
#ifdef CONFIG_BGP_SUPPRESS_CHECKSUMS
			/*
			 * Don't bother fixing up the udp checksum if it
			 * isn't being verified inside bluegene.
			 */
#else /* CONFIG_BGP_SUPPRESS_CHECKSUMS */
			{
			    unsigned int sum;
			    sum = (~ udp->check) & 0xffff;
			    if ((udp->len & 1) == 0) {
#define EVEN(n) ((n >> 16) + (n & 0xffff))
				sum += (EVEN(0xadded001u) + EVEN(seqno) +
					EVEN(tbu) + EVEN(tbl));
			    } else {
#define ODD(n) ((n >> 24) + ((n >> 8) & 0xffff) + ((n << 8) & 0xff00))
				sum += (ODD(0xadded001u) + ODD(seqno) +
					ODD(tbu) + ODD(tbl));
			    }
			    sum += 32;  /* length is included in csum twice */
			    while ((sum >> 16) != 0) {
				sum = (sum & 0x0000ffff) + (sum >> 16);
			    }
			    udp->check = (~ sum) & 0xffff;
			}
#endif /* CONFIG_BGP_SUPPRESS_CHECKSUMS */
			/*
			 * "len" has a lower bound of 60 (ETH_ZLEN), so for
			 * small packets we may not be changing it.
			 */
...
static int emac_poll_rx(void *param, int budget)
{
 ...
 again:
	while (budget > 0) {
		...
		if (MAL_IS_RX_EMPTY(&dev->rx_desc[slot]))
			break;
#endif
		skb = dev->rx_skb[slot];
		mb();
#ifdef CONFIG_IBM_NEW_EMAC_TOMAL
		len = dev->rx_desc[slot].frame_len;
		//printk("emac_poll_rx: desc: %p, slot=%d, len=%d\n", &dev->rx_desc[slot], slot, len);

		if (len == 0)
			break;

		if (!(dev->rx_desc[slot].status & TOMAL_RX_STATUS_LAST)) {
			printk("TOMAL: last packet not set\n");
		}
#else
		len = dev->rx_desc[slot].data_len;
		if (unlikely(!(((ctrl) & MAL_RX_CTRL_SINGLE) == MAL_RX_CTRL_SINGLE)))
			goto sg;

		ctrl &= EMAC_BAD_RX_MASK;
		if (unlikely(ctrl && ctrl != EMAC_RX_TAH_BAD_CSUM)) {
			mal_parse_rx_error(dev, &dev->rx_desc[slot]);
		...
		ip_summed = emac_rx_csum(dev, &dev->rx_desc[slot]);
#ifdef CONFIG_EMAC_MULTICAST_TAGGING
		len = mcast_tag(dev, skb, len);
#endif /* CONFIG_EMAC_MULTICAST_TAGGING */
...
static int __devinit emac_wait_deps(struct emac_instance *dev)
{
	...
	deps[EMAC_DEP_XGMII_IDX].phandle = dev->xgmii_ph;
	if (dev->tah_ph)
	...
	if (err == 0) {
		...
		dev->xgmii_dev = deps[EMAC_DEP_XGMII_IDX].ofdev;
...
static int __devinit emac_init_phy(struct emac_instance *dev)
{
	...
	if (dev->phy.features & SUPPORTED_Autoneg) {
		...
		if (f & SUPPORTED_10000baseT_Full) {
			speed = SPEED_10000;
			fd = DUPLEX_FULL;
		} else if (f & SUPPORTED_1000baseT_Full) {
			speed = SPEED_1000;
			fd = DUPLEX_FULL;
...
static int __devinit emac_init_config(struct emac_instance *dev)
{
	...
	if (emac_read_uint_prop(np, "xgmii-device", &dev->xgmii_ph, 0))
		dev->xgmii_ph = 0;
	if (emac_read_uint_prop(np, "xgmii-channel", &dev->xgmii_port, 0))
		dev->xgmii_port = 0xffffffff;
	...
	if (device_is_compatible(np, "ibm,xemac"))
	{
		dev->features |= EMAC_FTR_XEMAC
			| EMAC_FTR_INDIRECT_STACR
	...
	if (dev->xgmii_ph != 0) {
		...
		dev->features |= EMAC_FTR_HAS_XGMII;
#else
		printk(KERN_ERR "%s: XGMII support not enabled !\n",
		       np->full_name);
		return -ENXIO;
#endif
	}
				...
				const struct of_device_id *match)
{
	...
	dev->rx_size = emac_rx_size(ndev->mtu);
	...
	/* Attach to XGMII, if needed */
	if (emac_has_feature(dev, EMAC_FTR_HAS_XGMII) &&
	    (err = xgmii_attach(dev->xgmii_dev, dev->xgmii_port, dev->phy_mode)) != 0)
		goto err_detach_rgmii;

	/* Attach to TAH, if needed */
	if (emac_has_feature(dev, EMAC_FTR_HAS_TAH) &&
	    (err = tah_attach(dev->tah_dev, dev->tah_port)) != 0)
		goto err_detach_xgmii;
	...
	// announce IP checksumming
	if (emac_has_feature(dev, EMAC_FTR_XEMAC))
		ndev->features |= NETIF_F_IP_CSUM;
		//ndev->features |= NETIF_F_IP_CSUM | NETIF_F_SG | NETIF_F_HIGHDMA;
	...
	if (emac_phy_supports_gige(dev->phy_mode) ||
	    emac_phy_supports_10gige(dev->phy_mode))
	{
 ...
 err_detach_xgmii:
	if (emac_has_feature(dev, EMAC_FTR_HAS_XGMII))
		xgmii_detach(dev->xgmii_dev, dev->xgmii_port);
 err_detach_rgmii:
	if (emac_has_feature(dev, EMAC_FTR_HAS_RGMII))
